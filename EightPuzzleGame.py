...
# Your name: Selva Subramani Damodaran
...
class EightPuzzleGame:
    ...
    def start(self):
        ...
        # init_tile = np.array([[1, 2, 3], [5, 0, 6], [7, 8, 4]])
        ...
        print("Time taken for solving through Uninformed Search:", time_elapsed)

        time_start_informed = time.time()
        IS_solver = InformedSearchSolver(init, goal)
        IS_solver.run()
        time_elapsed_informed = time.time() - time_start_informed
        print("Time taken for solving through Informed Search:",
              time_elapsed_informed)
